...
- 2026-06-05 — **C-loop UX round 2**: (#1) new `AskTutor.astro` chat-launch bar added to C4 + C5 (ask the tutor anytime); (#3) C3 chat now poses an **inline practice MCQ** answered inside the chat (verdict + "আরেকটা / পুরো অনুশীলনে যাও →"); (#4) **modality memory** — remembered default (localStorage `eduai-modality`) opens on load + "⭐ এটাকে ডিফল্ট করো" with toast + hint. Build clean (60). Pushed → auto-deploy.
